...
public class FilmRepositoryImpl implements FilmRepository {
    ...
    public List<Film> searchFilms() {
        final String getTopFilmsQuery = ALL_FILMS_SQL_QUERY +
                "WHERE film_id IN (SELECT film_id FROM Likes GROUP BY film_id ORDER BY COUNT(user_id) DESC)";
        List<Film> searchFilms = addGenresInFilm(jdbcTemplate.query(getTopFilmsQuery, new FilmMapper()));
    ...
    public List<Film> searchFilmsByTitle(String query) {
        final String getTopFilmsQuery = ALL_FILMS_SQL_QUERY + "WHERE name LIKE ?";
        List<Film> searchFilms = addGenresInFilm(jdbcTemplate.query(getTopFilmsQuery, new FilmMapper(), new String[] {"%" + query + "%"}));
        return addDirectorToAllFilms(searchFilms);
    }
